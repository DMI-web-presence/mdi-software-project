...
const socialLinks = [
  ...
  { href: "https://www.youtube.com/@mdisoftware", label: "YouTube", icon: siYoutube },
];

export function SiteFooter() {
  return (
    <footer className="footer-cosmos relative overflow-hidden text-white" data-site-footer>
          ...
          <div className="footer-column pr-3 md:px-12">
            ...
            <div className="mt-4 grid gap-2 text-sm font-medium leading-5 text-white/88 sm:mt-5 sm:gap-3 sm:text-base sm:leading-normal md:mt-6 md:text-lg">
              {footerServices.map((service) => (
                <Link className="transition hover:text-signal" href={service.href} key={service.label}>
                  {service.label}
                </Link>
              ))}
            </div>
      ...
      <div className="relative z-10 border-t border-white/14">
        <div className="section-shell flex flex-col items-center gap-2.5 py-4 text-center text-xs font-medium text-white/46 sm:flex-row sm:justify-between sm:gap-4 sm:py-7 sm:text-left sm:text-sm">
          ...
          <div className="flex justify-center gap-4 sm:gap-6">
            <Link className="transition hover:text-white" href="/confidentialitate">
              Confidențialitate
            </Link>
            <span className="text-white/22" aria-hidden="true">
              |
            </span>
            <Link className="transition hover:text-white" href="/termeni">
              Termeni
            </Link>
          </div>
        </div>
